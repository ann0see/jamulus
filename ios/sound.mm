                                     ...
                                     AudioBufferList*            ioData )
{
    ...
    // copy output data
    for ( int i = 0; i < pSound->iDeviceBufferSize; i++ )
    {
...
bool CSound::init() // pgScorpio: Some of these function results could be stored and checked in checkCapabilities()
{
    ...
    try
    {
        iDeviceBufferSize = iRequestedBufferSize;
...
long CSound::createDeviceList ( bool )
{
    ...
    strDeviceNames.add ( "System Default In/Out Devices" )

        AVAudioSession* sessionInstance = [AVAudioSession sharedInstance];
    if ( sessionInstance.availableInputs.count > 1 )
    {
        strDeviceNames.add ( "in: Built-in Mic/out: System Default" );
    }
