...
void CServer::DecodeReceiveData ( const int iChanCnt, const int iNumClients )
{
    ...
    // get gains and pannings of all connected channels, compacted to the order
    // of "vecChanIDsCurConChan" (acquire the mutex once per channel instead of
    // once per channel pair; the second index of "vecvecfGains" does not
    // represent the channel ID)
